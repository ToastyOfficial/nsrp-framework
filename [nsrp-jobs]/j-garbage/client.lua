...
RegisterNetEvent('garbage:recPlayerLevel')
AddEventHandler('garbage:recPlayerLevel', function(level)
  pLevel = level
  --print(pLevel)
end)
...
Citizen.CreateThread(function()
	TriggerServerEvent('garbage:getLevel', GetPlayerServerId(PlayerId()))
  Wait(500)
  print(pLevel)
  ...
  for k, v in pairs(jobStarts) do
    ...
    if v.level <= pLevel then -- if job start level is lower than player's level then draw
      print(v.marker)
      makeBlip(v.marker.x, v.marker.y, 0, 318, 0, 'Job: Garbage')
    end
  end
    ...
    if not working then
      ...
      local heading = closestStart.h
        ...
        if closestStart.level < pLevel then
            ...
            showAlert('Press ~INPUT_PICKUP~ to start garbage route.', true)
              ...
              if not exports.nsrp_jobhandler:getHasJob() then -- if they don't already have a job
                ...
                showNotification('Go to the first pickup point.', 'info')
                Citizen.Wait(3000)
                showNotification('Press [ E ] to pickup trash.', 'info')
                Citizen.Wait(3000)
                showNotification('To cancel job, and return to yard, press [ J ].', 'info')
              else
  ...
  if working then
    ...
    for k, v in pairs(points) do
      if math.random(1, 100) > 33 then
        local coords = v.blip
        local blip = makeBlip(coords.x, coords.y, coords.z, 1, 14, 'Garbage Pickup')
		...
		if working then
  		...
  		if distanceFromGoal < markerDrawDistance then
				...
				if distanceFromGoal < 1 then
					showAlert('Press ~INPUT_PICKUP~ to pickup trash.', true)
					if IsControlJustReleased(0, 38) then
						RemoveBlip(currentGoal)
						-- give XP
						local reward = math.random(100,400)
						showNotification('Garbage picked up. Go to next location!', 'success')
						TriggerServerEvent('garbage:xp', GetPlayerServerId(PlayerId()), reward)
...
function getNextGoal()
	...
	if nextGoal then
		...
		showNotification('Drop off trash to start a new route.', 'success')

		local pickup = makeBlip(closestStart.pickup.x, closestStart.pickup.y, closestStart.pickup.z, 1, 14, 'Garbage Drop-off')
  		...
  		local distanceFromPickup = GetDistanceBetweenCoords(pedCoords, pickupCoords, false)
			...
			if distanceFromPickup < markerDrawDistance then
				...
				if distanceFromPickup < 1 then
					showAlert('Press ~INPUT_PICKUP~ to drop off trash.', true)
					if IsControlJustReleased(0, 38) then
            TriggerServerEvent('garbage:xp', GetPlayerServerId(PlayerId()), 1000)
            showNotification('Trash dropped off, next route started.', 'success')
						setupRoute()
						RemoveBlip(pickup)
						break
					end
...
RegisterCommand('garbage:abortjob', function()
     if working then
...
RegisterKeyMapping('garbage:abortjob', 'Reset Garbage Job', 'keyboard', 'j')
